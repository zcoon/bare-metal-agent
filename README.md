...
Lastly we included a safety loop to ensure we don't iterate more than 10 times to prevent budget overflow. The loop engine continues to run if the `stop_reason` is driven by a `tool_use`, or ends if it finds `end_turn` as a top reason